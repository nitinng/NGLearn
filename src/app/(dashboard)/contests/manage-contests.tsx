...
import { Loader2, Trash2, Edit, Plus, FolderTree, Activity, X, ChevronDown, Users } from "lucide-react";
...
export function ManageContests({
  ...
  // View States
  const [activeForm, setActiveForm] = useState<"none" | "series" | "sub">("none");
  ...
  // Edit State
  const [editMode, setEditMode] = useState<"series" | "sub" | null>(null);
  ...
  const handleCreateSubContest = async (e: React.FormEvent) => {
    ...
    if (!newSubStart || !newSubEnd) return toast.error("Start Date and End Date are required");
    if (newSubStart >= newSubEnd) return toast.error("Start Date must be before End Date");

    setIsSubmitting(true);
    const res = await createSubContest({
      name: newSubName,
      series_id: newSubSeriesId,
  ...
  const startEdit = (type: "series" | "sub", item: any) => {
    setActiveForm("none");
    setEditMode(type);
    setEditId(item.id);
  ...
  const handleUpdate = async () => {
    ...
    if (editMode === "series") {
      ...
      res = await updateSubContest(editId, {
        name: editData.name,
  ...
  const handleDelete = async (type: "series" | "sub", id: string) => {
    if (!confirm("Are you sure you want to delete this item? This action cannot be undone.")) return;
    setIsSubmitting(true);
    const res = type === "series" ? await deleteContestSeries(id) : await deleteSubContest(id);
    if (res?.error) toast.error(res.error);
      ...
      <div className="flex justify-between items-center pb-2 border-b border-border/40">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm font-medium">
            Configure series and sub-contests. Participants are sourced from the{" "}
            <Link href="/settings/members" className="text-primary hover:underline">Members list</Link>.
          </p>
        </div>
        <DropdownMenu>
              ...
              <form onSubmit={handleCreateSeries} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input id="s-name" value={newSeriesName} onChange={e => setNewSeriesName(e.target.value)} required placeholder="e.g. Learn Along with Coursera" className="bg-background/50 border-border/60 rounded-md" />
                  </div>
                  <div className="grid gap-2">
                          ...
                          <TableCell colSpan={3} className="p-4 border-b border-border/50">
                            <div className="flex flex-col gap-4 max-w-3xl">
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button variant="ghost" onClick={cancelEdit} disabled={isSubmitting} size="sm" className="rounded-md">Cancel</Button>
                              </div>
                            </div>
                          </TableCell>
                        </TableRow>
              ...
              <form onSubmit={handleCreateSubContest} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
                  <div className="grid gap-2">
                    ...
                    <Input type="date" id="sc-start" value={newSubStart} onChange={e => setNewSubStart(e.target.value)} required className="bg-background border-border/60 rounded-md" />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="sc-end" className="text-xs uppercase tracking-wider text-muted-foreground">End Date *</Label>
                    <Input type="date" id="sc-end" value={newSubEnd} onChange={e => setNewSubEnd(e.target.value)} required className="bg-background border-border/60 rounded-md" />
                  </div>
                </div>

                <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5" />
                  Participants are automatically sourced from the{" "}
                  <Link href="/settings/members" className="text-primary hover:underline">Members list</Link>.
                </p>
                    ...
                    <React.Fragment key={sc.id}>
                      <TableRow className="group hover:bg-transparent">
                        ...
                        <TableCell className="text-muted-foreground text-sm whitespace-nowrap">
                          {sc.start_date
                            ? <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-xs font-medium">{sc.start_date}</span>
                            : <span className="text-muted-foreground/50">—</span>
                          }
                        </TableCell>
                        <TableCell className="text-muted-foreground text-sm whitespace-nowrap">
                          {sc.end_date
                            ? <span className="px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-700 dark:text-blue-400 text-xs font-medium">{sc.end_date}</span>
                            : <span className="text-muted-foreground/50">—</span>
                          }
                        </TableCell>
                        <TableCell className="text-right">
